...
- **Pin drop input:** **Pin Drop From Global Screen Events** defaults **on** (tap/touch pin anywhere). Assign **Pin Drop UI Blocker Root** to the SIK / UI Starter screen hierarchy so `ScreenTransform.containsScreenPoint` skips pin placement on UI. Also: `pinDropInteraction` + `TriggerPrimaryEvent`. **Editor Touch Blocking For Preview** defaults off.
