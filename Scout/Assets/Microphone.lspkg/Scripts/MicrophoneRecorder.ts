...
export class MicrophoneRecorder extends BaseScriptComponent {
  ...
  private playbackDurationOverride: number = 0
  ...
  private onPlaybackAudio() {
    ...
    const duration = this.playbackDurationOverride > 0 ? this.playbackDurationOverride : this._recordingDuration
    this.currentPlaybackTime = Math.min(this.currentPlaybackTime, duration)
    ...
    if (this.currentPlaybackTime >= duration) {
      this.audioComponent.stop(false)
      this.playbackAudioUpdateEvent.enabled = false
      this.playbackDurationOverride = 0
    }
  }
  ...
  /** Returns a deep copy of the current recording for storing per-artifact. */
  public getRecordedFramesCopy(): AudioFrameData[] {
    const copy: AudioFrameData[] = []
    for (let i = 0; i < this.recordedAudioFrames.length; i++) {
      const f = this.recordedAudioFrames[i]
      copy.push({
        audioFrame: new Float32Array(f.audioFrame),
        audioFrameShape: f.audioFrameShape
      })
    }
    return copy
  }

  /** Play back a previously saved recording (e.g. linked to an artifact). */
  public playRecordedFrames(frames: AudioFrameData[]): boolean {
    if (frames.length <= 0) return false
    let totalSamples = 0
    for (let i = 0; i < frames.length; i++) {
      totalSamples += frames[i].audioFrameShape.x
    }
